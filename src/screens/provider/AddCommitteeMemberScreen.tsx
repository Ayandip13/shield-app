...
import { createCommitteeMember } from '../../services/committeeService';
import { Ionicons } from '@expo/vector-icons';

const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
const PHONE_REGEX = /^[+0-9\s\-()]{5,20}$/;

export const AddCommitteeMemberScreen: React.FC = () => {
  ...
  useEffect(() => {
    async function loadBuildings() {
      try {
        ...
        setErrors((prev) => ({
          ...prev,
          general: 'Failed to load buildings directory. Create a building first.',
        }));
      } finally {
  ...
  const validateForm = (): boolean => {
    ...
    if (!buildingId) {
      newErrors.buildingId = 'Please select an assigned building';
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrors({
        general: err.message || 'Failed to create committee member account.',
      });
    } finally {
        ...
        >
          <Card variant="elevated" style={styles.formCard}>
            <View style={styles.cardHeader}>
              <Ionicons name="person-add" size={28} color={theme.colors.committee} />
              <Text variant="title" style={styles.headerTitle}>
                Register Committee Member
              </Text>
            </View>
            <Text variant="caption" style={styles.headerSubtitle}>
              Create a committee portal account for building management representatives.
            </Text>
            ...
            {isLoadingBuildings ? (
              ...
              <View style={styles.formGroup}>
                ...
                <Input
                  label="Email Address *"
                  placeholder="e.g. committee.greenview@secureguard.com"
                  value={email}
                  onChangeText={(text) => {
                    setEmail(text);
            ...
            <View style={styles.buttonRow}>
              ...
              />
              <Button
                title={isSubmitting ? 'Creating...' : 'Save Member'}
                variant="primary"
