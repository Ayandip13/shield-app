...
type EditRouteProp = RouteProp<ProviderStackParamList, 'EditBuilding'>;

const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
const PHONE_REGEX = /^[+0-9\s\-()]{5,20}$/;

export const EditBuildingScreen: React.FC = () => {
  ...
  const [contactPhone, setContactPhone] = useState<string>(building.contactPhone || '');
  const [contactEmail, setContactEmail] = useState<string>(building.contactEmail || '');
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      setErrors({
        general: err.message || 'Failed to update building. Please check your network connection.',
      });
    } finally {
        ...
        >
          <Card variant="elevated" style={styles.formCard}>
            <View style={styles.cardHeader}>
              <Ionicons name="create" size={28} color={theme.colors.primary} />
              <Text variant="title" style={styles.headerTitle}>
                Edit Building Info
              </Text>
            </View>
            <Text variant="caption" style={styles.headerSubtitle}>
              Update contact or location details for {building.name}.
            </Text>
            ...
            <View style={styles.buttonRow}>
              ...
              />
              <Button
                title={isSubmitting ? 'Saving...' : 'Update Building'}
                variant="primary"
